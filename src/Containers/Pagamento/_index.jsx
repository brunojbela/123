...
class Pagamento extends Component {
	...
	sendPayment() {
			...
			if (this.state.termos) {
				if (Order.data.orderId && !this.checkData(this.data)) {
					...
					this.props.setCreditPayment(Order.data.orderId, [this.data]), false;

					var result = this.props.setCreditPayment(Order.data.orderId, [this.data], true);
					// var data_test = this.data;
					// var order_test = Order.data.orderId;

					result.then( (valor) => {

						this.interval = setInterval(() => {
							console.log('teste');
							// result = this.props.setCreditPayment(order_test, [data_test]);
							console.log(result)
							if(valor === 'loading') {
								...
								this.setState({
									loading: false,
									error: 'Ouve um problema no pagamento, por favor revise os dados informados e tente novamente. Caso o erro persista entre em contato no numero 0800 591 0697',
								});
							}else if(valor === 'false'){
								console.log('teste');
							}
	...
	render() {
		...
		if (Payment.data.paymentId) { 
            if(Payment.data.paymentMethod === "boleto" && Order.data.planId === 1) {
